<?php
// ═══════════════════════════════════════════════════════════════════
// WMS LOGÍSTICA — index.php
// Serve o React App sem necessidade de build/Node.js
// Dependências carregadas via CDN: React 18, Babel, xlsx, pdf-lib
// ═══════════════════════════════════════════════════════════════════

// Segurança básica: bloqueia acesso direto a arquivos sensíveis
...
// ── Info do servidor (opcional, para debug) ──────────────────────
...
<head>
  ...
  <meta name="description" content="WMS Logística — Sistema Operacional de Gestão de Cargas" />
  <title>WMS Logística</title>
  <link rel="icon" type="image/svg+xml" href="data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 32 32'><rect width='32' height='32' rx='6' fill='%230a0f1e'/><text y='24' x='4' font-size='20'>📦</text></svg>" />
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* Tela de loading enquanto React inicializa */
    #loading-screen {
      position: fixed;
      inset: 0;
      background: #070b14;
    ...
    .loading-logo span { color: #3b82f6; }
...
<body>
  ...
  <div id="loading-screen">
    <div class="loading-logo">WMS <span>LOGÍSTICA</span></div>
    <div class="loading-bar-track">
      <div class="loading-bar-fill"></div>
    </div>
    <div class="loading-sub">CARREGANDO SISTEMA OPERACIONAL...</div>
    <!-- Info PHP discreta (remove em produção se quiser) -->
    <div style="position:absolute;bottom:12px;right:16px;font-size:9px;color:#1e293b;letter-spacing:0.1em;">
      PHP <?= htmlspecialchars($php_version) ?> · <?= htmlspecialchars($server_time) ?>
    </div>
  </div>
  ...
  <div id="error-screen">
    ...
    <div style="font-size:11px;color:#64748b;">Verifique sua conexão e recarregue a página.</div>
    <button onclick="location.reload()" style="margin-top:8px;padding:6px 16px;background:#1e40af;color:#fff;border:none;border-radius:6px;cursor:pointer;font-family:inherit;font-size:11px;">
      ↻ Recarregar
    </button>
  </div>
  ...
  <!-- ── CDN: Babel Standalone (transpila JSX no browser) ──────── -->
  <script src="https://cdnjs.cloudflare.com/ajax/libs/babel-standalone/7.24.7/babel.min.js"></script>

  <!-- ── CDN: SheetJS (xlsx) ───────────────────────────────────── -->
  <script src="https://cdnjs.cloudflare.com/ajax/libs/xlsx/0.18.5/xlsx.full.min.js"></script>

  <!-- ── CDN: pdf-lib via skypack ─────────────────────────────── -->
  <!-- pdf-lib não tem UMD confiável no cdnjs; carregamos via esm.sh como módulo -->
  <script type="module">
    import { PDFDocument } from 'https://esm.sh/pdf-lib@1.17.1';
    window.PDFDocumentLib = PDFDocument;
  </script>

  <!-- ── App React (JSX transpilado pelo Babel in-browser) ─────── -->
  <script type="text/babel" data-presets="react" data-plugins="transform-modules-umd">
    // Shims para imports estáticos usados no JSX
    const { useState, useEffect, useCallback, useRef } = React;
    const XLSX = window.XLSX;

    // pdf-lib é carregado via ESM module acima; aguardamos ficar disponível
    const getPDFDocument = () => window.PDFDocumentLib;
    ...
    const roundTon = (kg) => { const ton = kg / 1000; return Math.round(ton * 10) / 10; };
    ...
    function WMS() {
        ...
        try {
          ...
          const combined = agenda.map((a) => {
            ...
            return {
              ...
              desc_doc:       sapItems[0]?.descricao_documento || "",
              peso_liquido:   peso,
              centro:         sapItems[0]?.centro || "",
              tipo_carga:     sapItems[0]?.tipo_carga || "",
            };
          });
          setRows(combined);
      ...
      const todayRows = rows.filter(r => r.fim_agenda && new Date(r.fim_agenda).toDateString() === hoje);
      const janela    = todayRows.filter(r => r.fim_agenda && new Date(r.fim_agenda) <= now);

      const planejado    = janela.reduce((a,r) => a + r.peso_liquido, 0);
      const realizado    = janela.filter(r => ["EXPEDIDO","EM FATURAMENTO"].includes(r.status)).reduce((a,r) => a + r.peso_liquido, 0);
      ...
      const displayRows   = (tab==="ativas" ? ativasRows : expedidasRows)
        .filter(r => filterText ? r.dt.includes(filterText) || r.transportadora.toLowerCase().includes(filterText.toLowerCase()) : true);
      ...
      const handleAgendaFile = async (e) => {
        ...
        try {
          ...
          const filtered = raw.filter(row => {
            ...
            if (doca && doca.startsWith("DOCA_") && !doca.includes("_IFNT")) return false;
          ...
          for (let i = 0; i < records.length; i += 50) {
            await supaFetch("/agenda_importada", {
              method: "POST",
              headers: { Prefer: "resolution=merge-duplicates,return=minimal" },
              body: JSON.stringify(records.slice(i, i+50)),
            });
          }
      ...
      const handleSapFile = async (e) => {
        ...
        try {
          ...
          const records = raw.map(row => ({
            ...
            tipo_carga:         (row["Inf. Agenda Entrega"]||"").includes("PLT") ? "Paletizada" : "Estivada",
        ...
        } catch (err) { showToast("❌ Erro SAP: " + err.message, "error"); }
        setImporting(false);
      };
      ...
      const handleManualSave = async () => {
        ...
        try {
          ...
          setManualDt(""); setManualTransp(""); setModal(null); loadData();
        } catch(e) { showToast("❌ "+e.message,"error"); }
      };

      const handlePdfMerge = async () => {
        const PDFDocument = getPDFDocument();
        if (!PDFDocument) { showToast("⏳ pdf-lib ainda carregando, tente novamente.","error"); return; }
      ...
      const handleExport = () => {
        const headers = ["DIA","DT","TRANSPORTADORA","HORA CHEGADA","STATUS","FATURAMENTO","GRADE","FIM","Nº PORTARIA","DESC. DOCUMENTO","PESO (kg)","TIPO"];
      ...
      const statusBg = (s) => ({
        ...
        "EM FATURAMENTO":"#3b0764","EXPEDIDO":"#052e16","NO SHOW":"#450a0a",
        "VEICULO RECUSADO":"#431407","MOTORISTA FOI EMBORA":"#422006",
      }[s] || "#0f172a");

      const statusBorder = (s) => ({
        "AG CHEGADA":"#334155","PATIO":"#1e40af","SEPARANDO":"#92400e","CARREGANDO":"#065f46",
        "EM FATURAMENTO":"#6b21a8","EXPEDIDO":"#14532d","NO SHOW":"#7f1d1d",
        "VEICULO RECUSADO":"#7c2d12","MOTORISTA FOI EMBORA":"#713f12",
      }[s] || "#1e293b");

      // ── RENDER ───────────────────────────────────────────────
      return (
        <div style={{minHeight:"100vh",background:"#070b14",color:"#e2e8f0",fontFamily:"'JetBrains Mono','Fira Code',monospace"}}>
          <style>{`
            *{box-sizing:border-box}
            ...
            .kpi-card:hover{transform:translateY(-2px);box-shadow:0 8px 32px rgba(0,0,0,.5)}
            ...
            .tag{display:inline-block;padding:2px 8px;border-radius:4px;font-size:10px;font-weight:600;letter-spacing:.05em}
          ...
          <div style={{background:"linear-gradient(90deg,#0a0f1e,#111827)",borderBottom:"1px solid #1e293b",padding:"10px 20px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
            <div style={{display:"flex",alignItems:"center",gap:16}}>
              ...
              <div>
                ...
                <div style={{fontSize:9,color:"#64748b",letterSpacing:"0.2em"}}>SISTEMA OPERACIONAL DE GESTÃO DE CARGAS</div>
              </div>
            </div>
            <div style={{display:"flex",alignItems:"center",gap:12}}>
              <div style={{textAlign:"right"}}>
                ...
                <div style={{fontSize:9,color:"#64748b"}}>{now.toLocaleDateString("pt-BR",{weekday:"short",day:"2-digit",month:"2-digit",year:"numeric"}).toUpperCase()}</div>
              </div>
          ...
          <div style={{padding:"12px 20px",display:"grid",gridTemplateColumns:"repeat(8,1fr)",gap:8}}>
            {[
              ...
              {label:"PRÉ FATURA",   value:null, color:"#f97316", editable:true},
          ...
          <div style={{padding:"0 20px 8px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
            <div style={{display:"flex",gap:4}}>
              {["ativas","expedidas"].map(t=>(
                <button key={t} className="btn" onClick={()=>setTab(t)} style={{background:tab===t?"#1e40af":"#1e293b",color:tab===t?"#fff":"#64748b",padding:"6px 16px",borderRadius:6,fontSize:11,fontWeight:600,border:tab===t?"1px solid #3b82f6":"1px solid #1e293b",letterSpacing:"0.08em"}}>
                  {t.toUpperCase()} <span style={{fontSize:9,marginLeft:4,opacity:.7}}>({t==="ativas"?ativasRows.length:expedidasRows.length})</span>
                </button>
              ))}
            </div>
            <div style={{display:"flex",gap:8,alignItems:"center"}}>
              ...
              <div style={{fontSize:9,color:"#334155"}}>CTRL+M · CTRL+Y · CTRL+O · CTRL+D</div>
            </div>
          </div>
                  ...
                  {displayRows.map((row,i)=>(
                    <tr key={row.dt} className="row-hover" style={{background:i%2===0?"#0a0f1e":"#080d1a",borderBottom:"1px solid #0f172a"}}>
                      ...
                      <td style={{padding:"4px 6px"}}>
                        <select value={row.status} onChange={e=>upsertOp(row.dt,"status",e.target.value,row.status)} style={{background:statusBg(row.status),color:"#e2e8f0",border:`1px solid ${statusBorder(row.status)}`,borderRadius:4,padding:"3px 4px",fontSize:10,fontFamily:"inherit",width:"100%",cursor:"pointer"}}>
                      ...
                      <td style={{padding:"4px 6px"}}><input value={row.grade} onChange={e=>upsertOp(row.dt,"grade_inicio",e.target.value,row.grade)} style={{background:"#0f172a",border:"1px solid #1e293b",borderRadius:4,padding:"3px 6px",color:"#94a3b8",fontSize:10,width:70,fontFamily:"inherit"}}/></td>
                      <td style={{padding:"4px 6px"}}><input value={row.fim} onChange={e=>upsertOp(row.dt,"grade_fim",e.target.value,row.fim)} placeholder="HH:MM" style={{background:"#0f172a",border:"1px solid #1e293b",borderRadius:4,padding:"3px 6px",color:"#94a3b8",fontSize:10,width:60,fontFamily:"inherit"}}/></td>
                      <td style={{padding:"4px 6px"}}><input value={row.portaria} onChange={e=>upsertOp(row.dt,"portaria",e.target.value,row.portaria)} style={{background:"#0f172a",border:"1px solid #1e293b",borderRadius:4,padding:"3px 6px",color:"#94a3b8",fontSize:10,width:70,fontFamily:"inherit"}}/></td>
          ...
          {/* MODALS */}
          {modal && (
            <div className="modal-overlay" onClick={()=>setModal(null)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,.75)",backdropFilter:"blur(4px)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000}}>
              <div className="modal-box" onClick={e=>e.stopPropagation()} style={{background:"#0f172a",border:"1px solid #1e293b",borderRadius:12,padding:24,minWidth:480,maxWidth:"90vw",maxHeight:"85vh",overflowY:"auto"}}>
                ...
                {/* IMPORT */}
                {modal==="import-agenda" && (<>
                  <div style={{fontFamily:"'Rajdhani',sans-serif",fontSize:18,fontWeight:700,color:"#f1f5f9",marginBottom:16,display:"flex",alignItems:"center",gap:8}}><span style={{color:"#3b82f6"}}>⬆</span> IMPORTAÇÃO DE DADOS</div>
                  ...
                  {importStep===1 && (<>
                    <div style={{fontSize:12,color:"#64748b",marginBottom:12}}>Arquivo da Agenda da Transportadora (.xlsx / .xls)</div>
                    <div style={{fontSize:10,color:"#334155",marginBottom:16,padding:"8px 12px",background:"#0a0f1e",borderRadius:6,border:"1px solid #1e293b"}}>
                      Filtros: LOCAL → termina em 1110 ou 1111 · DOCA → _IFNT ou NULL · DATA → D+0 e D+1
                    </div>
                  ...
                  {importStep===2 && (<>
                    <div style={{fontSize:12,color:"#64748b",marginBottom:12}}>Arquivo SAP ZLES002 (.xlsx / .xls)</div>
                    <div style={{fontSize:10,color:"#334155",marginBottom:16,padding:"8px 12px",background:"#0a0f1e",borderRadius:6,border:"1px solid #1e293b"}}>
                      Cruzamento: Nº transporte → DT · Inf. Agenda Entrega → PLT=Paletizada
                    </div>
                ...
                {modal==="pdf" && (<>
                  ...
                  <div onDragOver={e=>e.preventDefault()} onDrop={e=>{e.preventDefault();setPdfFiles(prev=>[...prev,...Array.from(e.dataTransfer.files).filter(f=>f.type==="application/pdf")]);}} style={{border:"2px dashed #334155",borderRadius:8,padding:24,textAlign:"center",color:"#475569",fontSize:12,marginBottom:12}}>
                    Arraste PDFs aqui ou <label style={{color:"#3b82f6",cursor:"pointer",marginLeft:4}}>clique para selecionar<input type="file" accept=".pdf" multiple onChange={e=>setPdfFiles(prev=>[...prev,...Array.from(e.target.files)])} style={{display:"none"}}/></label>
                  </div>
                  {pdfFiles.length>0 && <div style={{marginBottom:12}}>{pdfFiles.map((f,i)=>(
                    <div key={i} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"4px 8px",background:"#1e293b",borderRadius:4,marginBottom:4,fontSize:11}}>
                      ...
                      <button className="btn" onClick={()=>setPdfFiles(prev=>prev.filter((_,j)=>j!==i))} style={{background:"none",color:"#ef4444",fontSize:16}}>×</button>
                    </div>
                  ))}</div>}
                  <div style={{marginBottom:12}}>
                    <label style={{fontSize:10,color:"#64748b",display:"block",marginBottom:4}}>NOME DO ARQUIVO</label>
                  ...
                  <div style={{display:"flex",gap:8}}>
                    <button className="btn" onClick={handlePdfMerge} style={{flex:1,background:"#1e40af",color:"#fff",padding:"8px",borderRadius:6,fontSize:11,fontWeight:600}}>⬇ BAIXAR PDF</button>
                ...
                {modal==="logs" && (<>
                  ...
                  <div style={{marginTop:8,fontSize:9,color:"#334155",textAlign:"right"}}>{logs[0]?.data_alteracao?new Date(logs[0].data_alteracao).toLocaleString("pt-BR"):""}</div>
                </>)}
    ...
    reactRoot.render(React.createElement(WMS));

    // Esconde loading screen após mount
    requestAnimationFrame(() => {
      setTimeout(() => {
        const ls = document.getElementById("loading-screen");
        if (ls) ls.classList.add("hidden");
        setTimeout(() => { if (ls) ls.remove(); }, 500);
      }, 800);
    });
  ...
  <!-- Fallback se Babel ou React falharem -->
  <noscript>
    <div style="display:flex;align-items:center;justify-content:center;height:100vh;background:#070b14;color:#ef4444;font-family:monospace;font-size:14px;text-align:center;padding:20px;">
      ⚠️ JavaScript é necessário para executar o WMS Logística.<br/>
      <span style="color:#64748b;font-size:11px;margin-top:8px;display:block;">Habilite o JavaScript no seu navegador.</span>
    </div>
  </noscript>
